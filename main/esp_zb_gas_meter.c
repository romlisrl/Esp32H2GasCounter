...
// Load counter value from NVS after gm_nvs_init
esp_err_t gm_counter_load_nvs()
{
    ...
    esp_err_t err = nvs_get_u64(my_nvs_handle, NVS_KEY, &saved_count);
    if (err == ESP_OK) {
...
esp_err_t gm_deep_sleep_init()
{
    ...
    switch (wake_up_cause)
    {
        ...
        case ESP_SLEEP_WAKEUP_EXT1:
        {
            ...
            { 
                ...
                {
                    // Проснулись от нового импульса — считаем всегда
                    gm_counter_increment(false);
                    // Если пин ещё LOW — помечаем чтобы не посчитать повторно
                    exception_pulse_button_on_hold = (level == 0);
                }
                else
                {
                    // Засыпали с LOW пином
                    if (level == 1)
                    {
                        // Пин вернулся в HIGH — сбрасываем флаг, новый импульс не считаем
                        exception_pulse_button_on_hold = false;
                    }
                    // level == 0: пин всё ещё LOW — ждём, не считаем
                }

                check_reel_state_change_is_ok(level); // синхронизируем состояние для ISR
                resolved = true;
            }
            if (!resolved)
            {
    ...
    check_shall_measure_battery();
        ...
        ESP_ERROR_CHECK(esp_sleep_enable_ext1_wakeup(gpio_mainbtn_pin_mask | gpio_pulse_pin_mask , ESP_EXT1_WAKEUP_ANY_LOW));
...
void IRAM_ATTR gpio_pulse_isr_handler(void *arg)
{
    ...
    if (is_ok && level == 0)  // for interrupt to GND changed to 0
    {
...
esp_err_t gm_gpio_interrup_init()
{
    ...
    // RTC-уровневый pull держится во время deep sleep:
    gpio_pullup_en(PULSE_PIN);
    gpio_pulldown_dis(PULSE_PIN);
    gpio_pullup_en(MAIN_BTN);
    gpio_pulldown_dis(MAIN_BTN);
...
esp_err_t esp_zb_power_save_init(void)
{
    ...
    esp_pm_config_t pm_config = {
        ...
        .min_freq_mhz = 32,  // ESP32H2: valid options are 32, 64, 96 MHz (80 MHz is C6-only)
    ...
    esp_pm_config_t pm_config = {
        .max_freq_mhz = cur_cpu_freq_mhz,
        .min_freq_mhz = 32,  // ESP32H2: valid options are 32, 64, 96 MHz
        .light_sleep_enable = true
    };
    #endif
    #else
